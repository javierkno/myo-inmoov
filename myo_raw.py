...
class BT(object):
    ...
    def write_attr_no_wait(self, con, attr, val):
        self.send_command(4, 5, pack('BHB', con, attr, len(val)) + val, False)
        #return self.wait_event(4, 1)
...
class MyoRaw(object):
    ...
    def write_attr_no_wait(self, attr, val):
        if self.conn is not None:
            self.bt.write_attr_no_wait(self.conn, attr, val)
    ...
    def deep_sleep(self):
    		self.write_attr_no_wait(0x19, pack('2B', 0x04, 0x00))
